...
import { OrderDAO } from "../DAO/DB/OrderDAO.Database";
...
export class PlaceOrder {
    ...
    async execute(account_id: string, param: PlaceOrderParam) {
        ...
        const paymentAssetWallet = await this.wallet.getWallet(account, paymentAsset);
        const assetWallet = await this.wallet.getWallet(account, asset);

        debug('paymentAsset (wallet):', paymentAssetWallet.quantity, paymentAsset.ticker)
        debug('asset (wallet):', assetWallet.quantity, asset.ticker)
        debug(`${param.side}: Total = ${param.price} * ${param.quantity} = ${param.price * param.quantity}`)
        ...
        if (side == 'buy' && paymentAssetWallet.quantity < param.quantity * param.price)
            throw new Error(ERROR_MESSAGE.INSUFFICIENT_FUNDS);

        if (side == 'sell' && assetWallet.quantity < param.quantity)
            throw new Error(ERROR_MESSAGE.INSUFFICIENT_FUNDS);

        return await this.order.create(account, asset, paymentAsset, side, param.quantity, param.price);
    }
}